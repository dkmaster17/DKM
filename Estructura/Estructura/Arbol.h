...
class Arbol
{
	...
	public:
		...
		void imprimirConNivel( NodoA<T>*,int ) const; // imprime la ListaD en el flujo std de salida en un nivel especifico
	    
	    int getCantDesc( NodoA<T>* recor, int nodos ) const;
...
template<class T> void Arbol<T>::imprimirConNivel( NodoA<T>* recor ,int nivel) const
{
	// si recor es NULL
	if( !recor )
	{
	...
	// si el NodoA recor tiene hijo por la izquierda 
	if ( recor->getLink_hi() ) // paso recursivo para actualizar al hijo por la izquierda
		imprimirConNivel( recor->getLink_hi(),nivel );
	...
	// si el NodoA recor tiene hijo por la derecha
	if ( recor->getLink_hd() ) // paso recursivo para actualizar al hijo por la derecha
		imprimirConNivel( recor->getLink_hd() ,nivel);
} // fin de imprimir
