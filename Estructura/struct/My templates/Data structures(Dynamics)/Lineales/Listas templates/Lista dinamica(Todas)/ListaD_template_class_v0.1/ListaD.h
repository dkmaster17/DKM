 ...
 *	tipo lista dinamica, que puede representar cualquier de 
...
class ListaD
{
	...
	public:
		...
		static const int AGRE_CRE = 3; // agregar oreden creciente
		static const int AGRE_DECRE = 4; // agregar en orden decreciente
		static const int BUSCAR_POS_CRE = -1; // busca en orden creciente 
		static const int BUSCAR_POS_DECRE = 1; // busca en orden decreciente   
		static const int BUSCAR_ELEMENTO = 0; // buscar elemento 
		static const int ELIM_INICIO = -1; // eliminar al incio
		static const int ELIM_ELEMENTO = 0; // eliminar elemento(buscarlo) 
		static const int ELIM_FINAL = 1; // eliminar al final
		
		/**
		 *	@brief inicializa la ListaD, asiganando a NULL
		 * 	la cabezera y el fin de la misma, y crea una lista 
		 *	enlazada asignando a false sus atributos de tipo de
		 *	de lista DOBLE y CIRCU
		 *	@since 0.1
		 */
		ListaD()
		{	// inicia....
			this->cab = this->fin = NULL; // cab y fin
			this->DOBLE = this->CIRCU = false; // atributos de tipo de lista
		} // fin del constructor 
		 ...
		 *	@brief inicializa la ListaD, asiganando a NULL
		 * 	la cabezera y el fin de la misma, y crea una lista 
		 *	que depende de sus atributos de tipo de lista  
		 * 	DOBLE y CIRCU
		 *	@param bool especifica si la lista es doblemente(true) 
		 *	enlazada o no(false)
		...
		int agregar( T*, int pos = AGRE_CRE, bool repeticion = true ); // agrega un nuevo Nodo a la ListaD
...
/**
 *	@brief retorna una referencia a la cab de la ListaD
 *	@since: 0.1
 *	@return Nodo<T>* referencia a la cab de la ListaD
 */
...
/**
 *	@brief retorna una referencia al fin de la lista
 *	@since: 0.1
 *	@return Nodo<T>* referencia al fin de la ListaD
 */
...
/**
 *	@brief retorna el atributo CIRCU de la ListaD
 *	@since: 0.1
...
/**
 *	@brief retorna el atributo DOBLE de la ListaD
 *	@since: 0.1
...
/**
 *	@brief busca el elemento de información especificado o el lugar en la lista del elemento
 *	de información especificado, según sea el tipo comparación especificada
 ...
 *	parámetro la comparación sera por defecto de igualdad.
 * 	@param Nodo<T>** referencia al antes(Solo util para cuando se quiere agregar o eliminar)
 *	@since: 0.1
 *	@return Nodo<T>* referencia al campo buscado, Si el campo a buscar no se encuentra o la lista
 *	esta vacía la referencia apunatará a NULL o será falsa
 */
template<class T> Nodo<T>* ListaD<T>::buscar( T* info, int tipo, Nodo<T>** antes ) const
{
	// incluir un tercer tipo de busqueda para la funcion miembro eliminar 
	// que compare con == 
 ...
 *	nuevo elemento; AGRE_INICIO inserta al inicio; AGRE_FIN inserta al final;
 *	AGRE_CRE inserta en oreden creciente; AGRE_DECRE inserta en orden decresiente.
 *	En caso de omitirse este parámetro, se insertara en orden 
 ...
 *	operador en la clase que representa el campo info, la repetición se evitara de acuerdo a
 *	el tipo o atributo de comparación en la sobrecarga de la clase   
 ...
 *	el Nodo final según se especifiquen los parámetros o no 
 *	@since: 0.1
 *	@param int tipo de eliminación en la ListaD; ELIM_INICIO se
 *	elimina el inicio de lalista; ELIM_FINAL se elimina el fin de 
 *	la lista; ELIM_ELEMENTO se elimina el elemento con el campo info
 ...
 *	este sera NULL y no se tomara en cuanta para la eliminación, esta 
 ...
 *	@return int 0 si no se pudo eliminar, debido a que la lista esta vacía;
 ...
 *	imprimiendo todos los enlaces de la lista, independienteente 
