...
{
	private:
		NodoA<T>* cab; // cabezera de la lista 
		NodoA<T>* fin; // fin de la lista
		...
		NodoA<T>* getElemento( int& ); // retorna apuntador al NodoA en la posicion especificado
	public:
		...
		NodoA<T>* getCab() const; // retorna la cab de la ListaD
		NodoA<T>* getFin() const; // retorna el fin de la ListaD
		int getSize() const; // retorna el tamaño de la lista
		NodoA<T>* getElemento_NodoA( int ); // retorna apuntador al NodoA en la posicion especificado
		...
		NodoA<T>* buscar( T*, int tipo = BUSCAR_ELEMENTO, NodoA<T>** antes = NULL ) const; // busca elemento en la ListaD 
 ...
 *	@since: 0.1
 *	@return NodoA<T>* apuntador a la cabezera de la ListaD
 */
template<class T> NodoA<T>* ListaD<T>::getCab() const
{
 ...
 *	@since: 0.1
 *	@return NodoA<T>* apuntador al fin de la ListaD
 */
template<class T> NodoA<T>* ListaD<T>::getFin() const
{
 ...
 *	negativo o desborde el tamaño de la lista se retornara NULL
 */
template<class T> NodoA<T>* ListaD<T>::getElemento_NodoA( int indice )
{
	// obtengo el NodoA<T> en el indice especificado
	NodoA<T>* elemento = this->getElemento( indice );
	return elemento; // retorna NodoA<T>*
} // fin de getElemento_NodoA


/**
 *	@brief retorna  el campo info del NodoA en el 
 *	indice especificado 
 *	@since: 1.1
 *	@param int& indice del elemento a buscar en la listaD,
 *	es decir, posición del elemento en la lista en orden creciente
 *	@return T* apuntador añ campo de información del NodoA<T> en el indice
 *	especificado, en caso de que el indice indicado sea
 *	negativo o desborde el tamaño de la lista se retornara NULL
 */
 ...
 * 	@param NodoA<T>** apuntador de puntero al antes(Solo util para cuando se quiere agregar)
 *	@since: 0.1
 *	@return NodoA<T>* apuntador al campo info buscado, Si el campo info a buscar no se encuentra o 
 *	la ListaD esta vacía, el apuntador sera NULL 
 */
template<class T> NodoA<T>* ListaD<T>::buscar( T* info, int tipo, NodoA<T>** antes ) const
{
	NodoA<T>* recor = this->cab; // recor empezara en cab
 ...
 *	@since: 0.1
 *	@param T* campo de informacion del nuevo NodoA
...
template<class T> int ListaD<T>::agregar( T* info, int pos, bool repeticion )
{
	...
	NodoA<T>* recor = this->cab; // recorrido empezara en cab
	...
	if( nuevo ) // si la asignacion de memoria tuvo exito
	{
		...
		{ 
			// obtengo las posiciones recor y antes  del nuevo NodoA
...
/**
 *	@brief elimina el NodoA con el campo info especificado, o 
 *	el NodoA inicial o final, según se especifiquen los parámetros o no 
 *	@since: 0.1
...
{
	NodoA<T>* eliminar = this->cab; // NodoA a eliminar de la lista
	NodoA<T>* antes = NULL; // NodoA anterior a eliminar 
	...
	{	
		...
		{
			// obtengo copia del campo info del ultimo NodoA
			T* NodoAFin = this->fin->getInfo();
			
			//obtengo las posiciones antes y eliminar del ultimo elemento
			eliminar = this->buscar(  NodoAFin, BUSCAR_ELEMENTO, &antes );
...
/**
 *	@brief elimina todos los NodoAs de la ListaD, vaciando
 *	la ListaD.
 *	@since: 0.1
...
{
	NodoA<T>* recor = this->cab; // recor empieza en cab cab
	NodoA<T>* eliminar = NULL; // eliminar será NULL
	...
	if( !this->isVacia() ) 
	{
		...
		{	
			...
			if( recor == this->fin )
			{
				delete recor; // elimino ultimo NodoA
...
template<class T> void ListaD<T>::imprimir() const
{
	...
	if( recor && this->cab == this->fin )
	{
		...
		for( int i = 0; i < ( this->DOBLE ? 2 : 1 ); i++ )
		{
			...
			if( this->CIRCU ) // imprimo enlace circular
				...
				std::cout << "-->NULL" << std::endl; // enlace utimo NodoA
		} // fin del for
	} // fin del if 
	...
	{		
		std::cout << *recor->getInfo() << "-->"; // imprime NodoA
		recor = recor->getLink_suc(); // recor pasa a su sucesor
		...
		if( recor == this->fin )
		{
			std::cout << *recor->getInfo(); // ultimo NodoA
...
template<class T> void ListaD<T>::imprimir( bool sobrecarga ) const
{
	...
	if( !recor )
	{
		std::cout << "NULL" << std::endl; // imprime enlace del ultimo NodoA
		return; // retorna a la funcion que hizo la llamada
	} // fin del if
	...
	if( recor && this->cab == this->fin )
		std::cout << recor->getLink_pre() << "<--" << *recor->getInfo() << 
			"-->" << recor->getLink_suc() << std::endl; // imprime NodoA y sus dos enlaces
	...
	{	
		std::cout << recor->getLink_pre() << "<--" << *recor->getInfo() << 
			"-->" << recor->getLink_suc(); // imprime NodoA y sus dos enlaces
		recor = recor->getLink_suc(); // recor pasa a su sucesor
		...
		if( recor == this->fin )
			...
			"-->" << recor->getLink_suc() ; // imprime ultimo NodoA y sus dos enlaces
